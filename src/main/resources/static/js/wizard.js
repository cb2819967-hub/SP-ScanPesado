...
async function finishWizard(){
  ...
  // Nota: wz-unidad no existe en el HTML, usualmente se toma del primer select del wizard
  const unidadSelect = document.querySelector('#m-wizard select');
  const user = typeof getCurrentUser === 'function' ? getCurrentUser() : null;

  const data = {
    folio: 'V-' + new Date().getFullYear() + '-' + Date.now().toString().slice(-3),
    unidad: unidadSelect?.value || 'Unidad Genérica',
    tecnico: user ? user.nombre : 'Técnico',
    resultado: 'APROBADO' // En una implementación real, esto se calcularía
  };

  try {
    if (typeof apiCreateVerificacion === 'function') {
